  ...
  pomeloClient_require.register('pomelonode-pomelo-jsclient-websocket/lib/pomelo-client.js', function (exports, require, module) {
    (function () {
      ...
      var onmessage = function (cb, event) {
        ...
        txt && this.log && myConsole(txt.indexOf('"code":500') >= 0 ? 'error' : 'log', '%c收到消息', 'color: #DFC149;border: 1px solid #ccc', txt); // 排除心跳包
      ...
      var onerror = function (event) {
        ...
        this.log && myConsole('error', 'socket error: ', event);
      };
      var onclose = function (event) {
        this.emit('close', event)
        this.emit('disconnect', event)
        this.log && myConsole('warn', 'socket close: ', event)
        if (this.reconnect && this.reconnectAttempts < this.maxReconnectAttempts) {
          this.connecting = true
          this.reconnectAttempts++
          this.log && myConsole('info', 'socket reconnect: ', this.reconnectAttempts)
          var self = this
      ...
      pro.initWebSocket = function (url, cb) {
        this.log && myConsole('info', 'connect to ' + url);
      ...
      pro.disconnect = function () {
        if (this.socket) {
          ...
          this.log && myConsole('info', 'disconnect');
          this.socket = null;
        }
      ...
      pro.request = function (route, msg, cb) {
        this.log && myConsole('log', '%c发出消息: ' + route, 'color: green;border: 1px solid #ccc', msg);
      ...
      pro.send = function (packet) {
        if (this.socket && this.socket.readyState === WebSocket.OPEN) {
          ...
          this.log && myConsole('warn', 'socket is not open: readyState ' + this.socket.readyState)
        }
      };
      ...
      function heartbeatTimeoutCb() {
        ...
        if (gap > this.gapThreshold) {
          ...
          this.log && myConsole('error', 'server heartbeat timeout');
